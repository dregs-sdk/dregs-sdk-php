...
- The **secret key** (`sk_…`) this SDK authenticates with acts as an administrator of the team
  ...
  never in source control, and never anywhere a browser can reach it.
...
Verify every webhook before acting on it, against the **raw request body**. `Dregs\Webhooks::verify()`
does the HMAC comparison with `hash_equals`, in constant time, and rejects stale payloads as replays.
Verifying a body you decoded and re-encoded will not match, and working around that by skipping
verification would leave the endpoint open to anyone who learns its URL. Read the raw bytes with
`$request->getContent()` in Laravel or Symfony, or `file_get_contents('php://input')` in plain PHP —
not `$_POST`, and not a parsed array you encoded again.
